...
(defn- exercise [s]
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "SLE" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:margin-of-preference-eligibility-ground (:engagement m))
           (:margin-of-preference-eligibility-ground (:engagement d))))
