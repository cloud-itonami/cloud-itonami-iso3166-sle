...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (SLE, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up margin-of-preference-invalid) ==")
    ...
    (println "== filing/submit eng-4 (margin-of-preference-invalid -> HARD hold: 15% exceeds Regulation 75(3)'s 12% ceiling) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tax-clearance-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tax-clearance-unverified -> HARD hold) ==")
