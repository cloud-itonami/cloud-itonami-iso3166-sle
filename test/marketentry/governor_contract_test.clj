  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Freetown Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Freetown Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest margin-of-preference-invalid-is-held-and-unoverridable
  (testing "declared 15% margin-of-preference exceeds Regulation 75(3)'s own 12% ceiling -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:margin-of-preference-invalid} (-> (store/ledger db) last :basis)))
...
(deftest tax-clearance-unverified-is-held-and-unoverridable
  (testing "unverified NRA tax-clearance record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tax-clearance-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
