(ns marketentry.marketentryllm
  ...
  the Sierra Leone (SLE) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "claims-margin-of-preference?=" (:claims-margin-of-preference? e)
                        " margin-of-preference-pct=" (:margin-of-preference-pct e)
                        " margin-of-preference-eligibility-ground=" (:margin-of-preference-eligibility-ground e)
                        " tax-clearance-verified?=" (:tax-clearance-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tax-clearance-record? e))
                              (:tax-clearance-verified? e)))
                   0.9 0.3)}))
